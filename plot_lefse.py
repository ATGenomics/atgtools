...
colors = ["#E64B35B2", "#00A087B2", "#4DBBD5B2", "#3C5488B2",
...
def read_params():
    ...
    parser.add_argument('--format', dest="format", choices=["png", "svg", "pdf"], default='png', type=str,
    ...
    parser.add_argument('--background_color', dest="back_color", type=str, choices=["k", "w"], default="w",
    ...
    parser.add_argument('--report_features', dest="report_features", default=False, action='store_true',
...
def plot_histo_hor(path, paramshor, datahor, bcl, report_features):
    ...
    fig = plt.figure(figsize=(paramshor['width'], ints + ht),
    ...
    l_align = {'horizontalalignment': 'left',
               'verticalalignment': 'baseline'}
    r_align = {'horizontalalignment': 'right',
               'verticalalignment': 'baseline'}
    added = []
    ...
    for i, r in enumerate(datahor['rows']):
        ...
        if m * (indcl * 2 - 1) < 0 and bcl:
            ...
            ax.text(-mv / 40.0,
                    float(i) - 0.3, rr, r_align,
                    size=paramshor['feature_font_size'],
                    color=paramshor['fore_color'])
    ax.set_title(paramshor['title'],
    ...
    plt.savefig(path, format=paramshor['format'],
                facecolor=paramshor['back_color'],
                edgecolor=paramshor['fore_color'],
                dpi=paramshor['dpi'])
    plt.close()
...
if __name__ == '__main__':
    ...
    plot_histo_hor(params['output_file'],
                   params, data, len(data['cls']) == 2,
